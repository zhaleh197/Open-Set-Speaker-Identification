...
Notebook 21: build and self-test the 3-second-chunk bundle.

v1 scored 0.9600 on the leaderboard with plain 1-NN on ECAPA. v2 adds the four
changes that survived measurement, and leaves out the four that did not.
...
Same encoders, same decision rule, same tau. The only change is chunking:
3 s x up to 20 instead of 6 s x up to 10, holding total coverage at 60 s.

Measured on 15 realistic folds, 3 s sits about +0.0010 above 6 s at EVERY tau
from 0.10 to 0.40 -- a uniform offset, which is a far more trustworthy reading
than comparing two noisy argmaxes. The CV tau curve is flat enough that its peak
moves between protocols, so tau stays at the leaderboard-fitted 0.32.

The files this targets: 73 recordings that were too short for a single 6 s chunk
and so got tiled -- the model saw the same audio twice and "averaging over
chunks" averaged nothing. They now yield 2-3 genuine views.

Costs roughly twice the judging time: 20 forward passes per file instead of 10.
"""
...
SR, CHUNK_S, MAX_CHUNKS, MIN_SPEECH = 16000, 3.0, 20, 1.0
UNKNOWN = "unknown"
...
def main():
    ...
    e2 = list(Path("/kaggle/input").rglob("emb_c3_ecapa.npy"))
    e5 = list(Path("/kaggle/input").rglob("emb_c3_resnet.npy"))
    if not e2 or not e5:
        raise SystemExit("attach notebook 19 (3 s embeddings) and 02 (meta.json)")
    emb_ec = np.load(e2[0]); emb_rn = np.load(e5[0])
    meta = json.load(open(list(Path("/kaggle/input").rglob("meta.json"))[0]))
    ...
    z = shutil.make_archive(str(OUT / "submission_c3"), "zip", str(BUNDLE))
